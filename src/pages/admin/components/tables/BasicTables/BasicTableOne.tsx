...
import { EntregaCard } from "../../../pages/Resultados/components/EntregaCard";
...
export default function GanadoresTable() {
  ...
  const [uuiRifaPublico, setUuipublico] = useState("");
  ...
  if (error) {
    return (
      <div className="p-6 text-center">
        <p className="text-red-500 mb-3">Error al cargar ganadores: {error}</p>
        <button
  ...
  return (
    <div>
      <div className="overflow-x-auto rounded-2xl border border-gray-100 dark:border-white/[0.05]">
        <Table>
          {/* Table Header */}
            ...
            {ganadores.map((ganador) => (
              <TableRow key={ganador.idGanador}>
                ...
                <TableCell className="px-4 py-3 text-start">
                  <button
                    onClick={() => {
                      setUuipublico(ganador.rifa.uuidPublico);
                      setOpen(true);
                    }}
                  >
                    {(() => {
                      const estado =
                        ESTADO_ENTREGA_CONFIG[ganador.estadoEntrega.nombre] ?? ESTADO_ENTREGA_CONFIG.PENDIENTE;
                      return (
      ...
      {uuiRifaPublico && (
        <EntregaCard isOpen={isOpen} uuirifa={uuiRifaPublico} onClose={handleCloseModal} />
      )}
    </div>
  );
} 